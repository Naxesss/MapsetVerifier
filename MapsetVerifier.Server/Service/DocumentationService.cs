...
public static class DocumentationService
{
    ...
    {
        var checks = CheckerRegistry.GetChecksWithId().Where(check => check.Value is GeneralCheck);
        ...
        var result = checks.Select(check =>
        {
            ...
            return new ApiDocumentationCheck(
                ...
                outcomes: check.Value.GetTemplates().Select(template => template.Value.Level)
            );
        });

        return result.OrderBy(check => check.Category).ThenBy(check => check.Description);
    }

    public static IEnumerable<ApiDocumentationCheck> GetBeatmapDocumentation(Beatmap.Mode mode)
    {
        var gamemodeChecks = CheckerRegistry
            .GetChecksWithId()
            .Where(check =>
    ...
    public static ApiDocumentationCheckDetails? GetDocumentationDetails(int checkId)
    {
        ...
        var descriptions = check
            .GetMetadata()
            .Documentation.Select(section =>
            {
        ...
        return new ApiDocumentationCheckDetails(description: fullDescription, outcomes: outcomes);
    }
}
