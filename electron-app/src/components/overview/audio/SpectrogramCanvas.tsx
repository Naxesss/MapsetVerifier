...
const SpectrogramCanvas: FunctionComponent<SpectrogramCanvasProps> = (props) => {
  ...
  const draw = useCallback(
    (ctx: CanvasRenderingContext2D, width: number, height: number) => {
      ...
      if (
        props.avgFreq &&
        props.avgFreq > 0 &&
        props.data.frequencyBins &&
        props.data.frequencyBins.length > 0
      ) {
        ...
        if (
          binIndex === -1 &&
          props.avgFreq > props.data.frequencyBins[props.data.frequencyBins.length - 1]
        ) {
  ...
  const maxFreq =
    props.data?.frequencyBins?.[props.data.frequencyBins.length - 1] ||
      ...
      <Flex direction="row" gap="md">
        ...
        <Box>
          <Box style={{ position: 'relative', paddingLeft: 60 }}>
            {/* Y-axis (Frequency) */}
            <FrequencyAxis minFreqHz={minFreq} maxFreqHz={maxFreq} />

            {/* Canvas visualization */}
            <AutoResizeCanvas draw={draw} fixedWidth={1600} fixedHeight={800} />
          </Box>
        ...
        <Box
          style={{ position: 'relative', display: 'flex', alignItems: 'stretch', paddingRight: 50 }}
        >
          <AutoResizeCanvas draw={drawColorScale} fixedWidth={30} fixedHeight={400} />
