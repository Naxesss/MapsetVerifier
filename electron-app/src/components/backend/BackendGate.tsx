...
declare global {
  interface Window { __BACKEND_GATE__?: { ready: boolean } }
}
...
const BackendGate: React.FC<BackendGateProps> = ({
  ...
  const attemptStart = useCallback(async (manual = false) => {
    ...
    if (window.__BACKEND_GATE__?.ready && !manual) {
      setStatus('ready'); setStage('ready'); setProgress(100);
      return;
    }
    ...
    setStatus('starting'); setStage('init'); setProgress(10);
    try {
      if (manual) {
      ...
      if (pre) {
        ...
        setStage('ready'); setProgress(100); setStatus('ready'); setErrorMsg(null);
        return;
      }
      setStage('health'); setProgress(40);
      ...
      while (Date.now() - start < healthTimeoutMs) {
        ...
        if (await isHealthy(BACKEND_BASE_URL, 2500)) { healthy = true; break; }
  ...
  useEffect(() => {
    setStatus('idle'); setStage('init'); setProgress(0);
