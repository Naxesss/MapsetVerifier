import { BeatmapAnalysisResult, DifficultyOverviewResult } from '../Types';
...
const BeatmapAnalysisApi = {
  ...
  getDifficultyOverview: async function getDifficultyOverview(request: BeatmapAnalysisRequest) {
    return apiFetch('/beatmap-analysis/difficulty', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
      ...
      if (response.ok) {
        return data as DifficultyOverviewResult;
      } else {
        const message = data?.message || data?.error || raw || `HTTP ${response.status}`;
        const stackTrace = data?.stackTrace;
        throw new FetchError(response, message, stackTrace);
