            ...
            {
                {
                    ...
                    new IssueTemplate(
                        Issue.Level.Problem,
                        "{1} {0} has an inaccurate normalized multiplier of {3}. Consider changing the multiplier to {2}.",
                        "difficulty",
                        "timestamp",
                        "correctMultiplier",
                        "currentMultiplier"
        ...
        public override IEnumerable<Issue> GetIssues(BeatmapSet beatmapSet)
        {
            ...
            foreach (var beatmap in maniaBeatmaps)
            {
                ...
                if (
                    ...
                    ) || keymodes.Contains(beatmap.DifficultySettings.circleSize)
                )
                    continue;
                        ...
                        else
                        {
                            ...
                            if (!almostEquals(currentMultiplier, correctMultiplier, 0.02))
                                yield return new Issue(
                                    GetTemplate("Normalization Problem"),
                                    beatmap,
                                    beatmap.MetadataSettings.version,
                                    Timestamp.Get(timingLine.Offset),
                                    correctMultiplier,
                                    currentMultiplier
                                );
                        }
