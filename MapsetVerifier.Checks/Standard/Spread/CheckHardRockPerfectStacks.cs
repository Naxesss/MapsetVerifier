...
namespace MapsetVerifier.Checks.Standard.Spread
{
    ...
    public class CheckHardRockPerfectStacks : BeatmapCheck
    {
        ...
        public override IEnumerable<Issue> GetIssues(Beatmap beatmap)
        {
                ...
                for (var j = i + 1; j < hitObjectCount; ++j)
                {
                    ...
                    if (position == otherPosition)
                    {
                        candidates.Add(new IssueCandidate
                        {
                            ...
                            IsPerfect = true,
                            RequiredStackLeniency = (int)Math.Ceiling(timeDifference / (hrFadeInTime * 0.1))
                        });
            ...
            {
                var bestCandidate = group.OrderByDescending(candidate => candidate.IsPerfect)
                ...
                var timestampHitObjects = group.SelectMany(candidate => new[] { candidate.FirstHitObject, candidate.SecondHitObject })
                    .Distinct()
                ...
                {
                    yield return new Issue(GetTemplate(bestCandidate.TemplateName), beatmap, Timestamp.Get(timestampHitObjects), bestCandidate.RequiredStackLeniency);

                    continue;
                }

                yield return new Issue(GetTemplate(bestCandidate.TemplateName), beatmap, Timestamp.Get(timestampHitObjects), bestCandidate.DistanceText);
            }
        }

        private static int? GetHighestApplicableDifficultyIndex(double timeDifference, IReadOnlyList<double> snapping)
        {
        ...
        private static float ParseDistance(string? distanceText) => float.TryParse(distanceText, out var distance) ? distance : float.MaxValue;

        private static IEnumerable<List<IssueCandidate>> GetIssueGroups(IReadOnlyList<IssueCandidate> candidates)
        {
            ...
            foreach (var candidate in candidates)
            {
                ...
                while (queue.Count > 0)
                {
                    ...
                    foreach (var connected in adjacency[current.FirstHitObject].Concat(adjacency[current.SecondHitObject]))
        ...
        private static Dictionary<Stackable, StackState> ApplyHardRockStacking(Beatmap beatmap, IReadOnlyList<Stackable> stackables)
        {
                    ...
                    for (var j = i + 1; j < stackables.Count; ++j)
                    {
                        ...
                        if (hitObject is Circle || otherHitObject is Circle)
                        {
                            if (ShouldStack(hitObject, otherHitObject, states, stackTimeThreshold))
                            {
                                ...
                                {
                                    states[otherHitObject].StackIndex = states[hitObject].StackIndex - 1;
                                    wasChanged = true;
                                    break;
                                }
        ...
        private static bool MeetsStackTime(Stackable stackable, Stackable otherStackable, double stackTimeThreshold) => otherStackable.time - stackable.time <= stackTimeThreshold;
        ...
        private static bool CanStack(Stackable stackable, Stackable otherStackable, double stackTimeThreshold) => MeetsStackTime(stackable, otherStackable, stackTimeThreshold) && MeetsStackDistance(stackable, otherStackable);
        private static bool IsStacked(Stackable stackable, Stackable otherStackable, IReadOnlyDictionary<Stackable, StackState> states, double stackTimeThreshold) => CanStack(stackable, otherStackable, stackTimeThreshold) && states[stackable].StackIndex == states[otherStackable].StackIndex + 1;
        private static bool ShouldStack(Stackable stackable, Stackable otherStackable, IReadOnlyDictionary<Stackable, StackState> states, double stackTimeThreshold) => CanStack(stackable, otherStackable, stackTimeThreshold) && !IsStacked(stackable, otherStackable, states, stackTimeThreshold);

        private static bool CanStackTail(Slider slider, Stackable stackable, double stackTimeThreshold)
        {
            var sliderReferencePosition = slider.EdgeAmount % 2 == 0 ? slider.UnstackedPosition : slider.UnstackedEndPosition;
            return MeetsStackTime(slider, stackable, stackTimeThreshold) && Vector2.DistanceSquared(stackable.UnstackedPosition, sliderReferencePosition) < 3 * 3 && slider.time < stackable.time;
        }

        private static bool IsStackedTail(Slider slider, Stackable stackable, IReadOnlyDictionary<Stackable, StackState> states, double stackTimeThreshold) => CanStackTail(slider, stackable, stackTimeThreshold) && states[slider].StackIndex == states[stackable].StackIndex + 1;
        private static bool ShouldStackTail(Slider slider, Stackable stackable, IReadOnlyDictionary<Stackable, StackState> states, double stackTimeThreshold) => CanStackTail(slider, stackable, stackTimeThreshold) && !IsStackedTail(slider, stackable, states, stackTimeThreshold);

        private static Vector2 GetHardRockPosition(HitObject hitObject, IReadOnlyDictionary<Stackable, StackState> states, float circleRadius) =>
            hitObject is Stackable stackable ? GetStackedPosition(stackable.UnstackedPosition, states[stackable].StackIndex, circleRadius) : hitObject.Position;

        private static Vector2 GetStackedPosition(Vector2 position, int stackIndex, float circleRadius) => new(position.X + stackIndex * circleRadius * -0.1f, position.Y + stackIndex * circleRadius * -0.1f);
        private static double GetHardRockFadeInTime(float approachRate) => DifficultySettings.DifficultyRange(GetHardRockApproachRate(approachRate), (450, 1200, 1800));
        private static double GetHardRockStackTimeThreshold(Beatmap beatmap) => GetHardRockFadeInTime(beatmap.DifficultySettings.approachRate) * beatmap.GeneralSettings.stackLeniency * 0.1;
        private static float GetHardRockApproachRate(float approachRate) => Math.Min(10, approachRate * 1.4f);
        private static float GetHardRockCircleSize(float circleSize) => Math.Min(10, circleSize * 1.3f);
    }
}