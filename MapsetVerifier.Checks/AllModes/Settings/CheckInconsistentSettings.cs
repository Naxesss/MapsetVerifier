    ...
    public class CheckInconsistentSettings : BeatmapSetCheck
    {
        ...
        [
            ...
            new(
                "Warning",
                "countdown speed",
                beatmap => beatmap.GeneralSettings.countdown,
            ...
            new(
                "Warning",
                "countdown offset",
                beatmap => beatmap.GeneralSettings.countdownBeatOffset,
                (beatmap, otherBeatmap, beatmapSet) =>
                    CountdownSettingCondition(beatmap, otherBeatmap, beatmapSet)
                    && beatmap.GetCountdownStartBeat() >= 0
                    && otherBeatmap.GetCountdownStartBeat() >= 0
            ...
            new(
                "Warning",
                "letterbox",
                beatmap => beatmap.GeneralSettings.letterbox,
                (beatmap, otherBeatmap, _) =>
                    beatmap.GeneralSettings.mode == otherBeatmap.GeneralSettings.mode
                    && beatmap.Breaks.Count > 0
                    && otherBeatmap.Breaks.Count > 0
            ),
            new(
                "Warning",
                "widescreen support",
                beatmap => beatmap.GeneralSettings.widescreenSupport,
                StoryboardCondition
            ),
            new(
                "Warning",
                "storyboard in front of combo fire",
                beatmap => beatmap.GeneralSettings.storyInFrontOfFire,
                StoryboardCondition
            ),
            new(
                "Warning",
                "usage of skin sprites in storyboard",
                beatmap => beatmap.GeneralSettings.useSkinSprites,
                StoryboardCondition
            ),
            new(
                "Warning",
                "difficulty-specific storyboard presence",
                beatmap => beatmap.HasDifficultySpecificStoryboard()
            ),
            new(
                "Warning",
                "epilepsy warning",
                beatmap => beatmap.GeneralSettings.epilepsyWarning,
                (beatmap, otherBeatmap, beatmapSet) =>
            ...
            new(
                "Minor",
                "slider tick rate",
                beatmap => beatmap.DifficultySettings.sliderTickRate,
                (beatmap, otherBeatmap, _) =>
                    beatmap.GeneralSettings.mode == otherBeatmap.GeneralSettings.mode
            ),
        ];
        ...
        public override IEnumerable<Issue> GetIssues(BeatmapSet beatmapSet)
        {
            ...
            foreach (
                var issue in GetInconsistency(beatmap, otherBeatmap, beatmapSet, inconsistency)
            )
                yield return issue;
        }

        private IEnumerable<Issue> GetInconsistency(
            Beatmap beatmap,
        ...
        {
            if (
                inconsistency.Condition != null
                && !inconsistency.Condition(beatmap, otherBeatmap, beatmapSet)
            )
                yield break;
            ...
            if (value != otherValue)
                yield return new Issue(
                    GetTemplate(inconsistency.Template),
                    beatmap,
                    inconsistency.Name,
                    value,
                    otherBeatmap,
