    ...
    public class CheckHitSoundFormat : GeneralCheck
    {
        ...
        public override IEnumerable<Issue> GetIssues(BeatmapSet beatmapSet)
        {
            foreach (var hitSoundFile in beatmapSet.HitSoundFiles)
            {
                ...
                try
                {
                    actualFormat = AudioFileCache.GetFormat(fullPath);
                }
                catch (Exception ex)
                {
                    exception = ex;
                ...
                {
                    if (
                        (ChannelType.Wave & actualFormat) == 0
                        && (ChannelType.OGG & actualFormat) == 0
                    )
                        yield return new Issue(
                            GetTemplate("Unexpected Format"),
                            null,
                            hitSoundFile,
                            AudioBASS.EnumToString(actualFormat)
                        );
                    else if (
                        !hitSoundFile.ToLower().EndsWith(".wav")
                        && !hitSoundFile.ToLower().EndsWith(".ogg")
                    )
                        yield return new Issue(
