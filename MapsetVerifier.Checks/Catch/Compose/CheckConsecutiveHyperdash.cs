    ...
    public class CheckConsecutiveHyperdash : BeatmapCheck
    {
        ...
        public override IEnumerable<Issue> GetIssues(Beatmap beatmap)
        {
            ...
            for (var i = 0; i < catchObjects.Count; i++)
            {
                ...
                {
                    foreach (
                        var issue in CheckTrackedHyperdashes(beatmap, next, trackedHyperdashObjects)
                    )
                        yield return issue;
        ...
        private IEnumerable<Issue> CheckTrackedHyperdashes(
            Beatmap beatmap,
            ICatchHitObject? last,
            List<ICatchHitObject> trackedHyperdashObjects
        )
        {
            ...
            var isInSlider = trackedHyperdashObjects.Any(obj =>
                obj is JuiceStream
                || (
                    obj is JuiceStream.JuiceStreamPart juiceStreamPart
                    && juiceStreamPart.Kind != JuiceStream.JuiceStreamPart.PartKind.Tail
                )
            );
            ...
            if (
                IsAnyHigherSnapped(trackedPlusLast, Beatmap.Difficulty.Hard)
                && count > ThresholdPlatterHigher
            )
            {
            ...
            if (
                IsAnyHigherSnapped(trackedPlusLast, Beatmap.Difficulty.Insane)
                && count > ThresholdRainHigher
            )
            {
                yield return new Issue(
                    GetTemplate("HigherConsecutive"),
        ...
        private static bool IsAnyHigherSnapped(
            List<ICatchHitObject> trackedPlusLast,
