...
public sealed class CheckTestContext : IDisposable
{
    ...
    public static CheckTestContext Create(string fixtureName)
    {
        ...
        if (!Directory.Exists(sourcePath))
            throw new DirectoryNotFoundException(
                $"Beatmap fixture '{fixtureName}' was not found at '{sourcePath}'."
            );
    ...
    public Beatmap GetBeatmap(string difficultyName) =>
        BeatmapSet.Beatmaps.FirstOrDefault(beatmap =>
            beatmap.MetadataSettings.version == difficultyName
        )
        ?? throw new InvalidOperationException(
            $"Beatmap '{difficultyName}' was not found in fixture '{BeatmapSet}'."
        );
    ...
    private static void CopyDirectory(string sourcePath, string destinationPath)
    {
        ...
        foreach (
            var directory in Directory.GetDirectories(sourcePath, "*", SearchOption.AllDirectories)
        )
