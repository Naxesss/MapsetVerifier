      ...
      <Stack gap="sm">
        <Group justify="space-between">
          <Group gap="xs">
            ...
            <Tooltip
              label="Audio bitrate must be 128-192 kbps for MP3 or 128-208 kbps for Ogg Vorbis"
              multiline
              w={250}
            >
        ...
        <Group gap="md" mb="xs">
          <Group gap={4}>
            <Box w={20} h={3} bg="red.6" style={{ borderRadius: 2 }} />
            <Text size="xs" c="dimmed">Max Allowed ({data.maxAllowedBitrate} kbps)</Text>
          </Group>
          <Group gap={4}>
            <Box w={20} h={3} bg="yellow.6" style={{ borderRadius: 2 }} />
            <Text size="xs" c="dimmed">Min Recommended ({data.minAllowedBitrate} kbps)</Text>
          </Group>
        </Group>
        {chartData.length > 0 ? (
          <Box pos="relative">
            <LineChart
              ...
              xAxisProps={{ tickMargin: 10, allowDecimals: false }}
              valueFormatter={(value) => `${value} kbps`}
              referenceLines={[
                {
                  y: data.maxAllowedBitrate,
                  label: `Max (${data.maxAllowedBitrate})`,
                  color: 'red.6',
                  labelPosition: 'insideTopRight'
                },
                {
                  y: data.minAllowedBitrate,
                  label: `Min (${data.minAllowedBitrate})`,
                  color: 'yellow.6',
        ...
        {data.isCompliant && (
          <Group gap="xs" p="xs" bg={theme.colors.dark[6]} style={{ borderRadius: theme.radius.sm }}>
            ...
            <Text size="xs" c="green.4">
              Bitrate is within acceptable range for ranking
            </Text>
          </Group>
        )}

        <Text size="xs" c="dimmed">{data.complianceMessage}</Text>
      </Stack>
    </Paper>
  );
}
